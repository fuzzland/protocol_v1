...
contract VulnsUnverified is ERC721URIStorage {
    ...
    constructor(address _ParentContract) ERC721("Unverified Vuln Report", "uVULN") {
    ...
    function mint(address receiver, string memory _ipfsHash, uint256 _projectId, uint256 _reward, uint256 _oracleId) external returns (uint256) {
        require(!IFuzzProjectDeployer(ParentContract).IsOracleFreezed(_projectId, _oracleId), "oracle freezed");
