...
# telemetry backend
docker $PREFIX build . -f ./deploy/telemetry-backend/Dockerfile $PLATFORM -t $REGISTRY/telemetry-backend:$CURRENT_HASH
docker push $REGISTRY/telemetry-backend:$CURRENT_HASH

sed "s/REPLACE_ME_HASH/$CURRENT_HASH/g" deploy/telemetry-backend/backend.yaml > .new.telemetry-backend.yaml

kubectl apply -f .new.telemetry-backend.yaml
kubectl expose deployment telemetry-backend --port=50051 --target-port=50051
...
docker buildx build . -f deploy/fuzzer/Dockerfile -t fuzzland/cli
docker push fuzzland/cli